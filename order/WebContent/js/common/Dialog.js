...
//var IMGFOLDERPATH = 'http://localhost:8080/3g_remark/images/dialog/';//图片路径配置
var IMGFOLDERPATH = 'http://localhost:8080/OMDWeb/images/dialog/';//图片路径配置
...
var $jE = {};
$jE.$jA = function(attr,ele) {
	ele = ele || this;
	ele = $j(ele);
	return ele.getAttribute?ele.getAttribute(attr):null;
}
$jE.getTopLevelWindow = function(){
	var pw = window;
	while(pw!=pw.parent){
		pw = pw.parent;
	}
...
$jE.hide = function(ele) {
	ele = ele || this;
	ele = $j(ele);
  ele.style.display = 'none';
}
$jE.show = function(ele) {
	ele = ele || this;
	ele = $j(ele);
  ele.style.display = '';
}
$jE.visible = function(ele) {
	ele = ele || this;
	ele = $j(ele);
...
if ( !this.ParentWindow ){
	...
	arr.push("           	<input id='_ButtonOK_"+this.ID+"'  type='button' value='返  回'>");
	arr.push("           	<input id='_ButtonCancel_"+this.ID+"'  type='button' onclick=\"Dialog.getInstance('"+this.ID+"').close();\" value='取 消'>");
	arr.push("            </div></td></tr>");
	arr.push("      </table><a href='#;' onfocus='$j(\"_forTab_"+this.ID+"\").focus();'></a></td>");
	...
	var bgdiv = pw.$j("_DialogBGDiv");
	if(!bgdiv){
		bgdiv = pw.document.createElement("div");
		bgdiv.id = "_DialogBGDiv";
		$jE.hide(bgdiv);
	 	pw.$jT("body")[0].appendChild(bgdiv);
		if(isIE6){
	...
	var div = pw.$j("_DialogDiv_"+this.ID);
	if(!div){
		...
		//div.setAttribute("dragStart","Dialog.dragStart");
	 	pw.$jT("body")[0].appendChild(div);
	}
	/*div.onmousedown = function(evt){
		var w = $jE.getTopLevelWindow();
	...
	if(this.innerElementId){
		var innerElement=$j(this.innerElementId);
		...
		if(isIE){
			...
			pw.$j("_Covering_"+this.ID).parentNode.appendChild(fragment)
		}else{
			pw.$j("_Covering_"+this.ID).parentNode.appendChild(innerElement)
		}
	}
	pw.$j("_DialogDiv_"+this.ID).DialogInstance = this;
	if(this.URL)
		pw.$j("_DialogFrame_"+this.ID).DialogInstance = this;
	pw.Drag.init(pw.$j("_draghandle_"+this.ID),pw.$j("_DialogDiv_"+this.ID));//注册拖拽方法
	if(!isIE){
		pw.$j("_DialogDiv_"+this.ID).dialogId=this.ID;
		pw.$j("_DialogDiv_"+this.ID).onDragStart = function(){$j("_Covering_"+this.dialogId).style.display=""}
		pw.$j("_DialogDiv_"+this.ID).onDragEnd = function(){$j("_Covering_"+this.dialogId).style.display="none"}
	}

	this.OKButton = pw.$j("_ButtonOK_"+this.ID);
	this.CancelButton = pw.$j("_ButtonCancel_"+this.ID);

	//显示标题图片
	if(this.ShowMessageRow){
		$jE.show(pw.$j("_MessageRow_"+this.ID));
		if(this.MessageTitle){
			pw.$j("_MessageTitle_"+this.ID).innerHTML = this.MessageTitle;
		}
		if(this.Message){
			pw.$j("_Message_"+this.ID).innerHTML = this.Message;
		}
	}

	//显示按钮栏
	if(!this.ShowButtonRow){
		pw.$j("_ButtonRow_"+this.ID).hide();
	}
	if(this.CancelEvent){
...
Dialog.prototype.close = function(){
	if(this.innerElementId){
		var innerElement=$jE.getTopLevelWindow().$j(this.innerElementId);
		...
		if(isIE){
			...
			fragment.innerHTML=innerElement.outerHTML;
			innerElement.outerHTML="";
			$jT("body")[0].appendChild(fragment)
		}else{
			$jT("body")[0].appendChild(innerElement)
		}

	}
	if(this.WindowFlag){
		this.ParentWindow.$jD = null;
		this.ParentWindow.$jDW = null;
		...
		if(this.AlertFlag){
			$jE.hide(pw.$j("_AlertBGDiv"));
		}
		if(!flag&&!this.AlertFlag){//此处是为处理弹出窗口被关闭后iframe立即被重定向时背景层不消失的问题
			pw.eval('window._OpacityFunc = function(){var w = $jE.getTopLevelWindow();$jE.hide(w.$j("_DialogBGDiv"));}');
			pw._OpacityFunc();
		}
...
Dialog.prototype.addButton = function(id,txt,func){
	...
	var pw = $jE.getTopLevelWindow();
	pw.$j("_DialogButtons_"+this.ID).$jT("input")[0].getParent("a").insertAdjacentHTML("beforeBegin",html);
	pw.$j("_Button_"+this.ID+"_"+id).onclick = func;
}
...
Dialog.alert = function(msg,func,w,h){
	...
	diag.show();
	pw.$j("_AlertBGDiv").style.display="";
	$jE.hide(pw.$j("_ButtonOK_"+diag.ID));
	var win = pw.$j("_DialogFrame_"+diag.ID).contentWindow;
	...
	pw.$j("_DialogButtons_"+diag.ID).style.textAlign = "center";
}
...
Dialog.confirm = function(msg,func1,func2,w,h){
	...
	diag.show();
	pw.$j("_AlertBGDiv").style.dispaly="";
	var win = pw.$j("_DialogFrame_"+diag.ID).contentWindow;
	var doc = win.document;
	doc.open();
	doc.write("<body oncontextmenu='return false;'></body>") ;
	...
	if(d){
		try{
			...
			if(!this.AlertFlag){
				...
				if(!flag){
					$jE.getTopLevelWindow().$j("_DialogBGDiv").style.opacity="0.4";
					$jE.getTopLevelWindow().$j("_DialogBGDiv").style.filter="alpha(opacity=40)";
				}
			}
			if(d.AlertFlag){
				$jE.show($jE.getTopLevelWindow().$j("_AlertBGDiv"));
			}
			if(d.ShowButtonRow&&$jE.visible(d.CancelButton)){
...
Dialog.setPosition=function(){
	...
	{
		pw.$j("_DialogDiv_"+DialogArr[i]).DialogInstance.setPosition();
	}
}
Dialog.prototype.setPosition=function(){
	...
	//pw.$j(this.bgdivID).style.width= sw + "px";
	pw.$j(this.bgdivID).style.height= sh + "px";
}
